...
class commandes{
    ...
    function afficherCart(int $id){
        $sql="select * from commande,panier,produit,produit_panier WHERE commande.id_panier=panier.id and panier.id=produit_panier.id_panier and produit.ref=produit_panier.id_produit and commande.id=".$id;
        $db = config::getConnexion();
        try{
            $liste = $db->query($sql);
            return $liste->fetchAll();
